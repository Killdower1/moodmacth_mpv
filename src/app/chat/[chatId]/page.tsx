...
type Msg = {
  id: string; senderId: string; type: "TEXT"|"IMAGE"|"LOCATION"|"SYSTEM"|"CALL";
  text?: string | null; imageUrl?: string | null; locLat?: number | null; locLng?: number | null; createdAt: string;
};
...
export default function ChatDetail({ params }: { params: { chatId: string } }) {
  ...
  const load = async () => {
    const [mRes, cRes] = await Promise.all([
      fetch(`/api/chats/${chatId}/messages`, { cache: "no-store" }),
      fetch(`/api/chats/${chatId}/consent`, { cache: "no-store" }),
    ]);
  ...
  const send = async () => {
    ...
    setText("");
    await fetch(`/api/chats/${chatId}/messages`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
    await load();
  };
  ...
  const toggleConsent = async (key: keyof NonNullable<ConsentMine>) => {
    ...
    setMine(next);
    await fetch(`/api/chats/${chatId}/consent`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify(next) });
    await load();
  };
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 h-[calc(100vh-80px)] flex flex-col">
      <div className="flex items-center gap-3 mb-3">
        <h1 className="text-xl font-semibold flex-1">Chat {chatId.slice(0,8)}</h1>
