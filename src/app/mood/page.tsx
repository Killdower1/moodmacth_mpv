...
function SwipeCard({
  ...
  // Expose API untuk tombol Like/Skip
  useEffect(() => {
    if (!isTop) return
  ...
  const onUp = (_e: React.PointerEvent<HTMLDivElement>) => {
    if (!isTop || !dragging.current) return
  ...
  return (
    <div
      ...
      onPointerUp={onUp}
      className={\bsolute inset-0 \ select-none cursor-grab active:cursor-grabbing\}
      style={style}
    >
      ...
      <div className=\"w-full max-w-sm rounded-2xl border bg-background p-4 shadow-xl\">
        ...
        <div className=\"mb-4\">
          ...
          <div className=\"flex flex-wrap gap-2\">
            {moodOptions.map((m) => (
              <button key={m} onClick={() => setMood(m)} className={\ounded-xl border px-3 py-1 text-sm \\}>{m}</button>
            ))}
          </div>
        </div>
        <div className=\"mb-4\">
        ...
        <div className=\"mb-4\">
          ...
          <div className=\"flex gap-2\">
            {([\"any\",\"male\",\"female\"] as const).map(g => (
              <button key={g} onClick={() => setGender(g)} className={\ounded-xl border px-3 py-1 text-sm capitalize \\}>{g}</button>
            ))}
          </div>
        </div>
