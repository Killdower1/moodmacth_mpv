      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden rounded-2xl">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          <div className="rounded-xl bg-black/40 backdrop-blur-md px-3 py-2">
            <h3 className="text-xl font-bold">{profile.name} <span className="text-white/70">• {profile.age}</span></h3>
            {profile.gender && <p className="text-white/70 text-sm">{profile.gender}</p>}
          </div>
        </div>
      </div>
      <div className="mt-4 flex items-center justify-center gap-4">
        <button aria-label="Dislike" onClick={onDislike} className="h-14 w-14 grid place-items-center rounded-full bg-white/10 border border-white/15 text-2xl">✕</button>
        <button aria-label="Like" onClick={onLike} className="h-14 w-14 grid place-items-center rounded-full bg-[#FFCD00] text-black text-2xl font-bold">♥</button>
      </div>
    </div>
  );
}
